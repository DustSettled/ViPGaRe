...
def cluster_gaussians(dataset: ModelParams, iteration: int, n_keys=10, visualization=True, max_time=0.5, smooth=0.1, scene_type='unknown'):
    ...
    with (torch.no_grad()):
        ...
        use_affine = getattr(dataset, 'use_affine', True)
        deform = DeformModel(dataset.max_time, light=dataset.light, physics_code=dataset.physics_code, use_affine=use_affine)
