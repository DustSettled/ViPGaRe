...
# Import torch.func for efficient Jacobian computation (PyTorch >= 2.0)
try:
    ...
    # Fallback to functorch for older PyTorch versions (deprecated)
    try:
        from functorch import vmap, jacrev
        HAS_TORCH_FUNC = False
        import warnings
        warnings.warn(
            "Using deprecated functorch. Please upgrade to PyTorch >= 2.0 to use torch.func",
            DeprecationWarning
        )
    except ImportError:
        raise ImportError(
            "Neither torch.func nor functorch is available. "
            "Please upgrade to PyTorch >= 2.0 or install functorch."
        )
...
class VelocityWarpper(nn.Module):
    ...
    def integrate_pos(self, deform_code, pos_init, t1, t2, dt_max, deform=False, rot=False):
        # Fixed: use proper logical operators instead of bitwise operators
        # Only one of deform or rot can be True, not both
        assert not (deform and rot), "deform and rot cannot be True simultaneously"
        ...
        while unfinished.any():
            ...
            if deform:
                ...
                # 修复：添加[unfinished]索引以保持维度一致
                velocity = self.vel_net.get_vel(deform_code[unfinished], pt_mid)
...
class SegVel(nn.Module):
    def __init__(self, deform_code_dim=8, hidden_dim=64, layers=4, encode_dim=3):
        ...
        # 修复：使用扁平结构而非嵌套Sequential，以匹配训练时保存的权重
        self.weight_net = nn.Sequential()
        self.weight_net.append(nn.Linear(in_dim, hidden_dim))
        self.weight_net.append(nn.SiLU())
        for i in range(layers - 1):
            self.weight_net.append(nn.Linear(hidden_dim, hidden_dim))
            self.weight_net.append(nn.SiLU())
        self.weight_net.append(nn.Linear(hidden_dim, 6 * self.K))
